...
class ViewController: NSViewController {
    ...
    private let positioningView = NSView()
    ...
    override func viewDidLoad() {
        ...
        if (defaults.bool(forKey: "switchKey")) {
            ...
            checkKey.state = .off
            closeKey = HotKey(key: .r, modifiers: [.command, .shift])
            restoreKey = HotKey(key: .s, modifiers: [.command, .shift])
        }
    ...
    func serialCodeInstance() -> NSViewController {
        let storyboard = NSStoryboard(name: NSStoryboard.Name("Main"), bundle: nil)
        let identifier = NSStoryboard.SceneIdentifier("serialCode")
    ...
    @objc func openURL() {
        let url = URL(string: "https://getrefreshapp.com")
        NSWorkspace.shared.open(url!)
    }
    
    @objc func showSerialCode() {
        positioningView.frame = CGRect(x: (view.frame.width - 210) / 2, y: view.frame.height / 2, width: 1, height: 1)
        view.addSubview(positioningView)
        popoverView.contentViewController = serialCodeInstance()
        popoverView.animates = false
        popoverView.behavior = .semitransient
        popoverView.show(relativeTo: .zero, of: positioningView, preferredEdge: .maxX)
    ...
    @objc func switchKey() {
        if (checkKey.state == .on) {
            ...
            closeKey = HotKey(key: .r, modifiers: [.command, .shift])
            restoreKey = HotKey(key: .l, modifiers: [.command, .shift])
        } else {
            checkKey.state = .on
    ...
    // Options menu
    func setUpMenu() {
        self.settingsMenu.addItem(NSMenuItem(title: "Visit product website", action: #selector(openURL), keyEquivalent: ""))
        ...
        self.settingsMenu.addItem(NSMenuItem(title: "Software license key", action: #selector(showSerialCode), keyEquivalent: ""))
        self.settingsMenu.addItem(NSMenuItem.separator())
    ...
    func saveSessionGlobal() {
        ...
        for runningApplication in NSWorkspace.shared.runningApplications {
            ...
            if ((ignoreFinder.state == .on && (runningApplication.localizedName != "Finder" && runningApplication.localizedName != "Activity Monitor" && runningApplication.localizedName != "System Preferences" && runningApplication.localizedName != "App Store")) || ignoreFinder.state == .off) {
                ...
                if (runningApplication.activationPolicy == .regular && runningApplication.localizedName != "Refresh" && runningApplication != runningApp) {
        ...
        if ((ignoreFinder.state == .on && (runningApp.localizedName != "Finder" && runningApp.localizedName != "Activity Monitor" && runningApp.localizedName != "System Preferences" && runningApp.localizedName != "App Store")) || ignoreFinder.state == .off) {
            if (runningApp.activationPolicy == .regular && runningApp.localizedName != "Refresh") {
